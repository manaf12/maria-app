...
export default function AdminDeclarationsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(e?.response?.data?.message || t("admin.declarations.errors.loadFailed"));
    } finally {
      setLoading(false);
    }
  }
  ...
  const filtered = useMemo(() => {
    ...
    return items
      ...
      .filter((d) => (yearFilter === "ALL" ? true : String(d.questionnaireSnapshot?.taxYear ?? "") === yearFilter))
      .filter((d) => {
        if (!q) return true;
        const name = `${d.clientProfile?.firstName ?? ""} ${d.clientProfile?.lastName ?? ""}`.toLowerCase();
        const email = (d.clientProfile?.user?.email ?? "").toLowerCase();
        const id = (d.id ?? "").toLowerCase();
        const year = String(d.questionnaireSnapshot?.taxYear ?? "").toLowerCase();
        return name.includes(q) || email.includes(q) || id.includes(q) || year.includes(q);
      });
  ...
  async function assignSelected() {
    ...
    try {
      ...
      // Professional UX: remove assigned from unassigned list if mode=unassigned
      setItems((prev) =>
        prev.map((d) =>
          selectedIds.includes(d.id)
            ? { ...d, assignedAdminId: adminId.trim(), assignedAt: new Date().toISOString() }
            : d
        )
      );
      ...
      clearSelection();
      if (mode === "unassigned") {
        setItems((prev) => prev.filter((d) => !d.assignedAdminId));
      }
    } catch (e: any) {
      setError(e?.response?.data?.message || t("admin.declarations.errors.assignFailed"));
    } finally {
      setAssigning(false);
    }
  }
  ...
  <div className="admin-shell">
    ...
    <div className="toolbar-card">
      <div className="toolbar-grid">
        ...
        <select className="select-base" value={mode} onChange={(e) => setMode(e.target.value as any)}>
          <option value="all">{t("admin.declarations.filter.all")}</option>
          <option value="unassigned">{t("admin.declarations.filter.unassigned")}</option>
          <option value="assigned">{t("admin.declarations.filter.assigned")}</option>
        </select>
        ...
        <select className="select-base" value={yearFilter} onChange={(e) => setYearFilter(e.target.value)}>
      ...
      <div className="toolbar-row">
        ...
        <div className="row-actions">
          <button className="btn-soft" type="button" onClick={selectAllVisible}>
    ...
    <div className="bulk-card">
      <div className="bulk-grid">
        <div className="muted">
          {t("admin.declarations.selectedCount", { count: selectedIds.length })}
        </div>
        ...
        >
          {assigning ? t("admin.declarations.assigning") : t("admin.declarations.assignSelected")}
        </button>
      </div>
    </div>
    ...
    {error ? (
      ...
      <div className="muted" style={{ padding: 16 }}>{t("admin.declarations.loading")}</div>
    ) : filtered.length === 0 ? (
      <div className="muted" style={{ padding: 16 }}>{t("admin.declarations.empty")}</div>
    ) : null
      // <ResultsTable/> 
    }
    ...
    <div className="results-card">
      ...
      <div className="table-wrap">
        {loading ? (
          <div style={{ padding: 16 }} className="muted">{t("admin.declarations.loading")}</div>
        ) : filtered.length === 0 ? (
          <div style={{ padding: 16 }} className="muted">{t("admin.declarations.empty")}</div>
        ) : (
          <table className="pro-table">
            <thead>
              <tr className="table-header-row">
                <th className="table-header-cell">{t("admin.declarations.columns.select")}</th>
                <th className="table-header-cell">{t("admin.declarations.columns.client")}</th>
                <th className="table-header-cell">{t("admin.declarations.columns.tax")}</th>
                <th className="table-header-cell">{t("admin.declarations.columns.status")}</th>
                <th className="table-header-cell">{t("admin.declarations.columns.step")}</th>
                <th className="table-header-cell">{t("admin.declarations.columns.created")}</th>
                <th className="table-header-cell">{t("admin.declarations.columns.updated")}</th>
                <th className="table-header-cell">{t("admin.declarations.columns.assignment")}</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((d) => (
